...
public class AutoFillTest extends BaseDbTest {
  ...
  private void insertData() {
    ...
    Date now = new Date();
    TEST_TRANSACTIONS[0] = new TransactionInfo("Transaction 0", now, 0, testAccountId, payeeId, null, catId1, methodCreditCardId);
    TEST_TRANSACTIONS[1] = new TransactionInfo("Transaction 1", now, 200, testAccountId, payeeId, null, catId, methodChequeId); //this is the transaction we should retrieve
    TEST_TRANSACTIONS[2] = new TransactionInfo("Transaction 2", now, -100, testAccountId, payeeId1, null, catId, methodCreditCardId);
