...
class ComposeTransactionList @JvmOverloads constructor(
    ...
    lateinit var headerData: Flow<HeaderData>
    ...
    override fun Content() {
        AppTheme(context = context) {
            ...
            val headerData = headerData.collectAsState(HeaderData.EMPTY)
            val collapsedHeaders = rememberStringSetPreference(
                keyName = "collapsedHeaders_${accountId}_${headerData.value.grouping}",
            ...
            LazyColumn(modifier = Modifier.padding(horizontal = 12.dp)) {
                ...
                for (index in 0 until itemCount) {
                    ...
                    val headerId = transaction?.let { headerData.value.calculateGroupId(it) }
                    ...
                    if (transaction !== null && headerId != lastHeader) {
                        ...
                        stickyHeader(key = headerId) {
                            headerData.value.groups[headerId]
                                ?.let {
                                    HeaderRenderer(
                                        headerData.value.grouping,
                                        it,
                                        headerData.value.dateInfo,
                                        isExpanded = !isGroupHidden,
                                    ) {
                                        if (isGroupHidden) {
                                            collapsedHeaders.value = collapsedHeaders.value - headerId.toString()
                                        } else {
                                            collapsedHeaders.value = collapsedHeaders.value + headerId.toString()
                                        }
                                    }
    ...
    fun HeaderRenderer(
        ...
        Box {
            if (grouping != Grouping.NONE) {
                ExpansionHandle(modifier = Modifier.align(Alignment.TopEnd), isExpanded = isExpanded, toggle = toggle)
            }
            Column(
