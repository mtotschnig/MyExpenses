...
enum class AccountType(val isAsset: Boolean) {
    ...
    companion object {
        ...
        fun fromQifName(qifName: String): AccountType {
            return when (qifName) {
                "Oth L" -> LIABILITY
                "Oth A" -> ASSET
                "CCard" -> CCARD
        ...
        fun sqlOrderExpression(): String {
            val result = StringBuilder("CASE " + DatabaseConstants.KEY_TYPE)
            for (type in entries) {
                result.append(" WHEN '").append(type.name).append("' THEN ").append(type.sortOrder)
            }
            result.append(" ELSE -1 END AS " + DatabaseConstants.KEY_SORT_KEY_TYPE)
