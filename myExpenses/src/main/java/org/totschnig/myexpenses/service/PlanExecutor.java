...
public class PlanExecutor extends IntentService {
  ...
  private static final String KEY_FORCE_IMMEDIATE = "force_immediate";
  ...
  public void onHandleIntent(Intent intent) {
    ...
    final long lastExecution = PrefKey.PLANNER_LAST_EXECUTION_TIMESTAMP.getLong(now - H24);
    ...
    Date nextRun = TimePreference.getScheduledTime(prefHandler, PrefKey.PLANNER_EXECUTION_TIME);
    log("Scheduling next run at %s", nextRun);
    setAlarm(this, nextRun.getTime(), false);
  }
  ...
  public static void setAlarm(Context ctx, long when, boolean force) {
    if (CALENDAR.hasPermission(ctx)) {
      PendingIntent pendingIntent = getPendingIntent(ctx, force);
      AlarmManager manager = (AlarmManager) ctx.getSystemService(ALARM_SERVICE);
      manager.set(AlarmManager.RTC, when, pendingIntent);
    }
  }

  private static PendingIntent getPendingIntent(Context ctx, boolean force) {
    final Intent intent = new Intent(ctx, PlanExecutor.class);
    intent.putExtra(KEY_FORCE_IMMEDIATE, force);
    return PendingIntent.getService(ctx, 0, intent, FLAG_UPDATE_CURRENT);
  }

  public static void cancelPlans(Context ctx) {
    PendingIntent pendingIntent = getPendingIntent(ctx, false);
    AlarmManager manager = (AlarmManager) ctx.getSystemService(ALARM_SERVICE);
