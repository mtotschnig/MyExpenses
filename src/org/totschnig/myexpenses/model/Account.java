...
public class Account extends Model {
  ...
  public static final String[] PROJECTION = new String[] {KEY_ROWID,KEY_LABEL,KEY_DESCRIPTION,KEY_OPENING_BALANCE,KEY_CURRENCY,KEY_COLOR,
    "(SELECT coalesce(sum(amount),0) FROM " + VIEW_COMMITED + "  WHERE account_id = accounts._id and amount>0 and transfer_peer is null) as sum_income",
    "(SELECT coalesce(abs(sum(amount)),0) FROM " + VIEW_COMMITED + "  WHERE account_id = accounts._id and amount<0 and transfer_peer is null) as sum_expenses",
    "(SELECT coalesce(sum(amount),0) FROM " + VIEW_COMMITED + "  WHERE account_id = accounts._id and transfer_peer is not null) as sum_transfer",
    "opening_balance + (SELECT coalesce(sum(amount),0) FROM " + VIEW_COMMITED + "  WHERE account_id = accounts._id) as current_balance"};
